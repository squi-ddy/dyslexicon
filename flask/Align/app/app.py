...
def predict():
    ...
    wav_file = open("temp-{}.wav".format(rand_text), "wb")
    wav_file.write(speech)
    wav_file.close()
    text = args['text']
    with open("text-{}.txt".format(rand_text), "w") as text_file:
        for i in text.split():
            text_file.write(i + "\n")
     
    config_string = u"task_language=eng|is_text_type=plain|os_task_file_format=json"
    task = Task(config_string=config_string)
    task.audio_file_path_absolute = os.getcwd() + "/temp-{}.wav".format(rand_text)
    task.text_file_path_absolute = os.getcwd() + "/text-{}.txt".format(rand_text)
    task.sync_map_file_path_absolute = os.getcwd() + "/{}.json".format(rand_text)
    ExecuteTask(task).execute()
    task.output_sync_map_file()
    os.remove("temp-{}.wav".format(rand_text))
    os.remove("text-{}.txt".format(rand_text))
