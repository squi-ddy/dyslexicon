...
export function ForumCard(props: {
    ...
    return (
        <Card
            {...props.styleProps}
            key={props.id}
            onClick={props.hasLink ? (() => {
                navigate(props.id)
            }) : undefined}
            w={props.cardWidth === "full" ? props.cardWidth : props.cardWidth + "px"}
        ...
        >
            ...
            <CardBody minH={0} display={"flex"} flexDir={"column"}>
                <VStack w={"full"} spacing={2} align={"left"} overflowX={"hidden"}>
                <Text
                    fontSize="sm"
                    whiteSpace={"pre-wrap"}
                    overflow={"hidden"}
                ...
                <VStack w={"full"} spacing={2} maxH={props.limitHeight ? 200 : undefined} overflowY={"scroll"} flexShrink={1}>
                ...
                <HStack w={"full"}>
                    ...
                    {commentText.length > 0 && (
                        <CustomTooltip label={"Post comment"} placement={"left"}>
                            <IconButton
                                onClick={(e) => {
                                    addComment(props.id, {body: commentText, by: getLoggedInUser()}) 
                                    props.triggerReload()
                                    e.stopPropagation()
                                }}
                                float={"right"}
