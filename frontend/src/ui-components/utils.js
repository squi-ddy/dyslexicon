...
export const useDataStoreCollection = ({ model, criteria, pagination }) => {
    ...
    const fetch = () => {
        ...
        const subscription = DataStore.observeQuery(
            ...
            (snapshot) =>
                setResult({ items: snapshot.items, isLoading: false }),
        ...
        if (value === undefined || value === "" || value === null) {
            if (validation.type === "Required") {
                return {
                    hasError: true,
                    errorMessage:
                        validation.validationMessage || "The value is required",
                }
            } else {
                return {
                    hasError: false,
...
const checkValidation = (value, validation) => {
    if (validation.numValues?.length) {
        switch (validation.type) {
            ...
            case "EqualTo":
                return {
                    ...
                    errorMessage:
                        validation.validationMessage ||
                        `The value must be equal to ${validation.numValues.join(
                            " or "
                        )}`,
                }
            default:
        ...
        switch (validation.type) {
            case "StartWith":
                return {
                    hasError: !validation.strValues.some((el) =>
                        value.startsWith(el)
                    ),
                    errorMessage:
                        validation.validationMessage ||
            ...
            case "NotContains":
                return {
                    hasError: !validation.strValues.every(
